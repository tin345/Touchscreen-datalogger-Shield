...
byte second_now=0; //present second
byte second_last=0; // past second_now
byte second_count=0; // second counter
byte minute_now=0; //present minute
byte minute_last=0; // past minute_now
byte minute_count=0; // minute counter

byte second_rate = 1; // sample rate in minutes
byte minute_rate = 1; // sample rate in minutes
bool SD_writen=false;
...
void setup() 
{
  ...
  LCD_graph_data();  //draw graph on first pass regardless of sample rates
}
...
void loop() 
{
  ...
  timenow();      // create timeString
        delay(750);    // maybe 750ms is enough, maybe not  +++++++++  FIX Later  +=+++++++++++++++++++++++++++++++++++++=
  temp_read();
  ...
  LCD_graph_data(); // graph every second ... or only on data save 


  // second sampel rate code - excutes code every second_rate
  if(second_count>=second_rate)
  {
    // Write SD and Graph based upon sample rate
    //Serial.print(F("SD Write, SD Count = 0"));
    //Serial.println();
    //SD_write();
    LCD_text();
    LCD_graph_data(); // graph every second ... or only on data save 
    SD_writen=false;  // clear SD write indicator


    second_count = 0; // reset count
  }// end if


  // Minute sampel rate code - excutes code every miniute_rate
  if(minute_count>=minute_rate)
  {
    // Write SD and Graph based upon sample rate
    Serial.print(F("SD Write, SD Count = 0"));
    Serial.println();
...
void LCD_text(void) {
      //  tft.fillScreen(ILI9341_BLACK);
  tft.setCursor(0, 0);
  tft.setTextColor(ILI9341_RED, ILI9341_BLACK);    tft.setTextSize(2);
  tft.print(F("Tmp1= "));
...
{
  timeString = "";  // Clear past strings
  tempString="";
  now = rtc.now();
  // create time and date strings
  dateString = String(now.year())+"/"+String(now.month())+"/"+String(now.day())+" ";
  timeString += String(now.hour())+":"+String(now.minute())+":"+String(now.second())+" ";
 
  // Update second_now and second_count
  second_now = now.second();    
  if (second_now != second_last) // if a second has passed then
  {
    second_count++;  // increment counter for SD wirte
    second_last=second_now;  // reset second check
    Serial.print("SD Counter++ = ");
    Serial.println(second_count);
  }
  
  // Update Minute_now and Minute_count
  minute_now = now.minute();    
  if (minute_now != minute_last) // if a minute has passed then
  {
    minute_count++;  // increment counter for SD wirte
    minute_last=minute_now;  // reset minute check
    SD_writen=false;  // clear SD write indicator
    Serial.print("SD Counter++ = ");
